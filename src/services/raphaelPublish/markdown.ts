/*
 * Adapted from raphael-publish/src/lib/markdown.ts (MIT, see ./LICENSE.md).
 * Plugin-side adjustments:
 * - Drop `import 'highlight.js/styles/github.css'`; we already inline hljs token colors
 *   in `applyTheme()`, so the CSS file is unnecessary at runtime and would fail
 *   esbuild.
 * - Keep `applyTheme()` and `preprocessMarkdown()` untouched in spirit so
 *   future upstream sync stays mechanical.
 */
...
export const md = new MarkdownIt({
	...
	highlight: function (str, lang) {
		...
		const dots =
			...
			'<span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background: #27c93f;"></span></div>';
 ...
 * ```
 *
 * CommonMark treats that as an empty list item plus a stray `<p>`; WeChat drafts
 * look like numbering with missing text on every other line.
 */
function mergeOrphanListMarkers(content: string): string {
	...
	for (let i = 0; i < 48; i += 1) {
		const before = s;
		s = s.replace(olAttached, "$1 $2");
		s = s.replace(ulAttached, "$1 $2");
		if (before === s) break;
	}
	return s;
}
...
export function preprocessMarkdown(content: string) {
	content = mergeOrphanListMarkers(content);
	...
	// Collapse blank lines between consecutive list items (loose -> tight).
	// Loose lists cause markdown-it to wrap each item content in <p>, which
	// produces extra blank lines in WeChat. Run until stable (multi-item lists).
	for (let _lt = 0; _lt < 12; _lt++) {
		const _ltBefore = content;
		content = content.replace(
			/(^[ \t]*(?:\d+[.)]\s+|[-*+]\s+)[^\n]*(?:\n[ \t]+[^\n]+)*)\n\n(?=[ \t]*(?:\d+[.)]\s+|[-*+]\s+))/gm,
			"$1\n",
		);
		if (content === _ltBefore) break;
	}
	return content;
}
...
export function applyTheme(html: string, themeId: string) {
	...
	Object.keys(style).forEach((selector) => {
		...
		elements.forEach((el) => {
			...
			el.setAttribute(
				...
				currentStyle +
					"; " +
					style[selector as keyof typeof style],
			);
		});
	});
	...
	doc.querySelectorAll("ul ul").forEach((ul) => {
		...
		ul.setAttribute(
			"style",
			`${currentStyle}; list-style-type: circle !important;`,
		);
	});
	doc.querySelectorAll("ul ul ul").forEach((ul) => {
		...
		ul.setAttribute(
			"style",
			`${currentStyle}; list-style-type: square !important;`,
		);
	});
	...
	doc.querySelectorAll("pre code, pre .hljs, .hljs").forEach(
		(codeNode) => {
	...
	headings.forEach((heading) => {
		Object.keys(headingInlineOverrides).forEach((tag) => {
			heading.querySelectorAll(tag).forEach((node) => {
				...
				node.setAttribute(
					"style",
					`${node.getAttribute("style") || ""}; ${override}`,
				);
			});
		});
	});
