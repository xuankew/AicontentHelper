...
def main() -> int:
    ...
    vcfg_raw = job.get("videoConfig")
    vcfg: dict[str, Any] = vcfg_raw if isinstance(vcfg_raw, dict) else {}
    ...
    # Required explicit carousel files: plugin 已在 TS 侧通过 Moka 重导视频用图。
    raw_files = job.get("cardImageFiles")
    explicit = [Path(str(x).strip()) for x in raw_files] if isinstance(raw_files, list) else []
    explicit = [p for p in explicit if str(p).strip() and p.is_file()]
    if not explicit:
        _err("缺少有效 cardImageFiles：请先由插件使用 Moka 模版导出视频用图片，再触发 render_video.py")
        return 1
    imgs = explicit
    used_card_dir = str(explicit[0].parent.resolve())
    _log(f"轮播使用显式 cardImageFiles {len(imgs)} 张，来源：{used_card_dir}")
    ...
    with tempfile.TemporaryDirectory() as tdx:
        ...
        for pname in ("douyin", "xiaohongshu", "shipinhao"):
            pcopy = plats.get(pname)
            if pcopy is None:
                continue
            if not isinstance(pcopy, dict):
                _err(f"platforms.{pname} 类型错误（应为 JSON 对象），跳过")
                continue
            ...
            # Allow caller to provide a ready-made open-frame PNG (e.g. the
            ...
            if op_override:
                ...
                if op_src.is_file():
                    shutil.copy2(op_src, op_png)
                    _log(f"使用外部首帧封面: {op_src}")
                else:
                    _log(f"openFramePngPath 不存在，回退 Pillow 首帧: {op_src}")
                    op_override = ""
            if not op_override:
            ...
            if op_override:
                # Moka 小红书封面通常是 1242×1660（3:4），用 fullbleed 模糊底
                # 铺满 9:16，避免黑边。
                _png_to_mp4_knowledge_fullbleed(op_png, open_sec, o_mp4)
            else:
    ...
    meta = {
        **{k: v for k, v in vcfg.items() if not str(k).startswith("_")},
        ...
        "cardImageFiles": [str(p) for p in imgs],
